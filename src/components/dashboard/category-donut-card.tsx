...
export function CategoryDonutCard({
  ...
  return (
    <Card>
      ...
      <CardContent>
        {total === 0 ? (
          <p className="py-fib13 text-center text-sm text-muted-foreground">
            No spending yet this month.
          </p>
        ) : (
          <DonutBody breakdown={breakdown} total={total} />
        )}
      </CardContent>
    </Card>
  );
...
function DonutBody({
  ...
  >((segments, entry) => {
    ...
    const dash = (entry.amount / total) * CIRCUMFERENCE;
    ...
    <div className="flex flex-col items-center gap-fib21 sm:flex-row">
      ...
      <ul className="flex w-full flex-col gap-fib5">
        {segments.slice(0, 6).map((segment) => (
          <li
            key={segment.category}
            className="flex items-center justify-between gap-fib8 text-sm"
          >
            <span className="flex min-w-0 items-center gap-fib5 truncate">
              <span
                className="h-2 w-2 shrink-0 rounded-full"
                style={{
                  backgroundColor: CATEGORY_META[segment.category].color,
                }}
                aria-hidden="true"
              />
              <span className="truncate">
                {CATEGORY_META[segment.category].label}
              </span>
            </span>
            <span className="shrink-0 font-mono text-muted-foreground">
              {formatCents(segment.amount)}
            </span>
          </li>
        ))}
      </ul>
